...
### v2.1.0 (最新)
- **使用模式配置**: 添加了个人模式和服务模式的配置系统，支持动态切换
  - 个人模式：性能最大化，无限制流、无缓存、无熔断、无并发限制
  - 服务模式：可调节性能，支持限流、缓存、熔断、并发控制
- **配置管理API**: 添加了完整的配置管理API，支持动态修改配置
- **多标签页管理**: 实现了浏览器多标签页管理，支持创建、切换、关闭标签页
- **并发请求处理**: 实现了请求队列和并发控制，支持优先级队列
- **会话管理**: 实现了会话隔离和管理，支持多用户并发访问
- **代理支持**: 添加了代理管理器，支持HTTP/HTTPS/SOCKS代理
- **请求限流**: 实现了基于IP的限流器，支持白名单和自定义限流规则
- **缓存机制**: 实现了响应缓存管理器，支持TTL和自动清理
- **熔断机制**: 实现了熔断器，支持自动恢复和健康检查
- **监控指标**: 实现了完整的监控指标收集器，包括请求、响应时间、错误、资源使用
- **负载均衡**: 实现了负载均衡器，支持多种策略（轮询、最少连接、加权、随机）
- **批量操作API**: 添加了批量删除提供商、批量切换API密钥、批量导入/导出API
- **统计和监控API**: 添加了完整的统计和监控API，包括API密钥统计、模型调用频率、提供商性能对比、实时监控
- **代码质量修复**: 修复了TypeScript配置，启用严格模式；移除了所有@ts-ignore；修复了ESLint错误
- **统一工具函数**: 创建了统一的数据库连接获取工具函数和日志记录工具函数
- **统一错误处理**: 统一了错误处理中间件，移除了重复代码
